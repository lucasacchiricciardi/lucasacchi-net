...
function retrieveAndDecompress(lang) {
  try {
    ...
    function getRelatedArticles(currentArticle, limit) {
      ...
      // OPTIMIZATION: Replaced O(n*m^2) filter/sort chain with a single pass O(n*m) loop
      // to calculate shared tags efficiently, reducing redundant array allocations.
      var scoredArticles = [];
      for (var i = 0; i < allArticles.length; i++) {
        ...
        if (sharedCount > 0) {
          scoredArticles.push({ article: a, shared: sharedCount });
        }
      }
